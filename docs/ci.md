...
Live login is skipped for pull requests from forks. Forked PRs never receive Proton credentials, session JSON, or the encrypted session cache key; they run offline CI only. Maintainers can run live checks after reviewing the contribution in a trusted branch.

Cache behavior:

- cache key: branch slug + six-hour bucket
- cache contents: encrypted minimized Playwright storage state
- encryption: AES-256-GCM via `PROTONMAIL_SESSION_CACHE_KEY` with at least 32 bytes of entropy
