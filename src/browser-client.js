...
const TWO_FACTOR_TEXT_RE = /\b(two[- ]factor|2fa|totp|authenticator|two[- ]step|verification code|security code)\b/iu;
...
function ensurePrivateDir(dirPath) {
  ...
  try {
    ...
    debugLog("Could not tighten directory permissions", { path: dirPath, ...errorDetails(error) });
  }
}
...
function getCooldownState(sessionFile) {
  ...
  try {
    ...
    debugLog("Ignoring unreadable login cooldown file", { path: filePath, ...errorDetails(error) });
    return { active: false };
  }
}
...
function debugLog(message, details = {}) {
  const enabled = /^(1|true|yes|debug)$/iu.test(String(process.env.PROTONMAIL_DEBUG || ""));
  if (!enabled) {
    return;
  }
  const suffix = Object.keys(details).length ? ` ${JSON.stringify(details)}` : "";
  console.debug(`[protonmail-browser] ${message}${suffix}`);
}
...
function writePrivateJsonFile(filePath, payload) {
  ...
  try {
    ...
    try {
      ...
      debugLog("Could not remove temporary private file", { path: tmpPath, ...errorDetails(cleanupError) });
    }
    throw error;
  }
}
...
async function hasAuthChallenge(page) {
  ...
  for (const selector of challengeSelectors) {
    try {
      ...
      debugLog("Auth challenge selector probe failed", { selector, ...errorDetails(error) });
    }
  }
...
async function hasTwoFactorChallenge(page) {
  const currentUrl = page.url();
  if (/\/(two-factor|totp)(\/|$|[?#])/iu.test(currentUrl)) {
    return true;
  }
  ...
  for (const selector of twoFactorSelectors) {
    try {
      ...
      debugLog("Two-factor selector probe failed", { selector, ...errorDetails(error) });
    }
  }

  return hasTwoFactorChallengeText(await getVisiblePageText(page));
}

function hasTwoFactorChallengeText(content) {
  return TWO_FACTOR_TEXT_RE.test(normalizeText(content));
}
...
async function getVisiblePageText(page) {
  try {
    ...
    debugLog("Could not read visible page text", errorDetails(error));
    return "";
  }
}

async function saveSession(context, sessionFile) {
  writePrivateJsonFile(sessionFile, await context.storageState());
}
...
async function hasInboxIndicators(page) {
  for (const selector of [MESSAGE_ROW_SELECTOR, '[data-testid*="compose"]', '[data-testid*="navigation-link:inbox"]']) {
    try {
      ...
      debugLog("Inbox indicator selector probe failed", { selector, ...errorDetails(error) });
    }
  }
  return false;
}
...
async function getPageContent(page) {
  try {
    ...
    debugLog("Could not read page content", errorDetails(error));
    return "";
  }
}
...
async function locateLoginEmailField(page, timeout = 15000) {
  ...
  for (const candidate of candidates) {
    try {
      ...
      debugLog("Login email field candidate did not become visible", errorDetails(error));
    }
  }
  return null;
}
...
async function locateLoginPasswordField(page, timeout = 10000) {
  ...
  for (const candidate of candidates) {
    try {
      ...
      debugLog("Login password field candidate did not become visible", errorDetails(error));
    }
  }
  return null;
}
...
async function locateSignInButton(page, timeout = 10000) {
  ...
  for (const candidate of candidates) {
    try {
      ...
      debugLog("Sign-in button candidate did not become visible", errorDetails(error));
    }
  }
  return null;
}
...
async function locateStaySignedInCheckbox(page) {
  ...
  for (const candidate of candidates) {
    try {
      ...
      debugLog("Stay-signed-in checkbox candidate probe failed", errorDetails(error));
    }
  }
  return null;
}
...
async function waitForInboxOrLogin(page, timeoutMs) {
  ...
  while (Date.now() - startedAt < timeoutMs) {
    const currentUrl = page.url();
    if (await hasTwoFactorChallenge(page)) {
...
async function getAlertTexts(page) {
  ...
  for (let index = 0; index < count; index += 1) {
    try {
      ...
      debugLog("Could not read alert text", { index, ...errorDetails(error) });
    }
  }
  return texts;
}
...
async function completeAppsPageIfNeeded(page) {
  ...
  for (const target of candidates) {
    try {
      ...
      debugLog("Apps page mail target probe failed", errorDetails(error));
    }
  }
  return false;
}
...
async function performLogin({ page, context, username, password, sessionFile, suppressCooldown = false }) {
  ...
  if (staySignedIn) {
    try {
      ...
      debugLog("Could not set stay-signed-in checkbox", errorDetails(error));
    }
  }
...
async function scanInbox(page, limit = 50) {
  await page.waitForSelector('[data-testid="message-list-loaded"]', { timeout: 10000 }).catch((error) => {
    debugLog("Timed out waiting for message list loaded marker", errorDetails(error));
  });
  ...
  while (count > 0 && count < limit && attempts < 10 && count !== previousCount) {
    ...
    await rows.nth(count - 1).scrollIntoViewIfNeeded().catch((error) => {
      debugLog("Could not scroll last inbox row into view", { index: count - 1, ...errorDetails(error) });
    });
    await delay(1000);
  ...
  for (let index = 0; index < Math.min(count, limit); index += 1) {
    try {
      ...
      debugLog("Could not read inbox row preview", { index, ...errorDetails(error) });
    }
  }
...
async function openMessage(page, index) {
  ...
  await locator.scrollIntoViewIfNeeded().catch((error) => {
    debugLog("Could not scroll message into view", { index, ...errorDetails(error) });
  });
  try {
    await locator.click({ timeout: 5000 });
  } catch (error) {
    debugLog("Message click failed; trying DOM click fallback", { index, ...errorDetails(error) });
    await locator.evaluate((node) => node.click()).catch((fallbackError) => {
      debugLog("Message DOM click fallback failed", { index, ...errorDetails(fallbackError) });
    });
  }
...
async function getOpenedMessageSubject(page, fallback) {
  for (const candidate of [page.locator('[data-testid*="subject"]').first(), page.locator('[role="region"] h1').first(), page.locator("h1").first()]) {
    try {
      ...
      debugLog("Message subject candidate probe failed", errorDetails(error));
    }
  }
...
async function expandOriginalMessageIfNeeded(frame) {
  ...
  try {
    ...
    debugLog("Could not expand original message block", errorDetails(error));
  }
}
