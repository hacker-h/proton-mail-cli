...
export class ProtonMailBrowserClient {
  ...
  async loginAndSaveSession(options = {}) {
    ...
    try {
      ...
      const result = resultWithError(error?.message || "Unexpected Proton Mail login failure", { debugEvents: getDebugEvents(page) });
      if (keepOpenOnError) {
        return resultWithSession(result, { browser, context, page, debug });
      }
      return result;
    } finally {
      if (!debug?.keepOpenOnError) {
  ...
  async #ensureLoggedIn(options = {}) {
    ...
    try {
      ...
      return await this.#closeOrKeep(resultWithError(error?.message || "Unexpected Proton Mail browser failure", {
        debugEvents: getDebugEvents(page),
      }), { browser, context, page, debug, keepOpenOnError });
    }
  }
  ...
  async #launch({ headless, storageState, debug = { enabled: false } }) {
    ...
    const persistentOptions = {
      headless: false,
      args: [...launchArgs, `--remote-debugging-port=${debug.cdpPort}`],
      userAgent: this.#options.userAgent,
      viewport: this.#options.viewport,
      storageState: storageState || undefined,
    };
