...
| Variable | Purpose |
|---|---|
| `PROTONMAIL_USERNAME` | Proton Mail username for explicit fresh-login flows. |
| `PROTONMAIL_PASSWORD` | Proton Mail password for explicit fresh-login flows. |
| `PROTONMAIL_ENV_FILE` | Absolute path to a trusted dotenv-style credentials file. |
| `PROTONMAIL_SESSION_FILE` | Browser storage-state JSON path. Treat as secret-bearing. |
| `PROTONMAIL_DEBUG` | Enables debug logging and debug browser behavior when set to `1` or `true`. |
| `PROTONMAIL_DEBUG_CDP_PORT` | CDP port for debug browser sessions. |
| `PROTONMAIL_DEBUG_PROFILE_DIR` | Isolated persistent debug browser profile directory. |
| `PROTONMAIL_DEBUG_CHROMIUM` | Chromium executable path override. |
| `PROTONMAIL_LIVE_TEST` | Enables env-gated live Proton regression tests. |
| `PROTONMAIL_LIVE_HEADLESS` | Set to `0` to run live tests with a visible browser. |
| `PROTONMAIL_LIVE_SESSION_FILE` | Temporary session file path used by CI live tests. |
| `PROTONMAIL_ALLOW_FRESH_LOGIN` | Allows live tests to use username/password instead of requiring a seeded session. |
| `PROTONMAIL_SESSION_JSON` | GitHub Actions secret containing minimized Playwright storage state. |
| `PROTONMAIL_SESSION_CACHE_KEY` | GitHub Actions secret for encrypted branch session caches; use at least 32 bytes of entropy. |

See [SECURITY.md](SECURITY.md), [DEBUG.md](DEBUG.md), and [docs/ci.md](docs/ci.md) before changing session or CI behavior.
